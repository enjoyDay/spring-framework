...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	// 这是保存beanDefinition对象的空间，key是beanName,由于是全局的存在并发，因此用ConcurrentHashMap
	...
	// 保存所有的beanDefinition 的name，按照注册顺序来
	...
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
		...
		// 因为beanDefinitionMap是全局变量，这里定会存在并发访问的情况(map使用了currentHashMap)
		BeanDefinition existingDefinition = this.beanDefinitionMap.get(beanName);
		// 处理已经注册的beanName情况
		if (existingDefinition != null) {
			// 如果对应的beanName已经注册且在配置配置中配置了bean不允许被覆盖，则抛出异常
	...
	public Object doResolveDependency(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		try {
			 ...
			 * 返回值的构成：key=匹配的beanName，value=beanName所对应的实例化后的bean（通过getBean(beanName)获得）
			 */
			Map<String, Object> matchingBeans = findAutowireCandidates(beanName, type, descriptor);
			if (matchingBeans.isEmpty()) {
				// 如果aotuwire的require属性为true，而找到的匹配项却为空则只能抛出异常
