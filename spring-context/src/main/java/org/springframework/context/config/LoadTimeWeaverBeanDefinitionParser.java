...
class LoadTimeWeaverBeanDefinitionParser extends AbstractSingleBeanDefinitionParser {
	...
	// 由父类AbstractSingleBeanDefinitionParser#parseInternal方法来调用此类的doParse
	@Override
	protected void doParse(Element element, ParserContext parserContext, BeanDefinitionBuilder builder) {
		builder.setRole(BeanDefinition.ROLE_INFRASTRUCTURE);

		// 是否开启了AspectJ功能
		// ASPECTJ_WEAVING_ATTRIBUTE = "aspectj-weaving"
		if (isAspectJWeavingEnabled(element.getAttribute(ASPECTJ_WEAVING_ATTRIBUTE), parserContext)) {
			...
			if (!parserContext.getRegistry().containsBeanDefinition(ASPECTJ_WEAVING_ENABLER_BEAN_NAME)) {
				...
				//			"org.springframework.context.weaving.AspectJWeavingEnabler"
				// AspectJWeavingEnabler类是对AspectJ处理的类，将此封装到BeanDefinition中,也就是将类路径注册进去
